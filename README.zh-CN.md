...
辅助脚本只使用 Python 标准库，负责采集证据；实际更新和清理由代理按用户授权，通过合适的维护工具完成。脚本本身不执行安装、删除、重启或服务启动。

**语言范围：** 本介绍提供四种语言版本。Skill 指令、详细参考和脚本生成的 Markdown 报告目前使用简体中文；CLI 参数和 JSON 字段名使用英文。
...
`git clone` 遇到非空的目标目录会停止。已有同名 Skill 时更新原安装，避免同时安装重复副本。Codex 通常会自动发现变更，未出现时重新启动。
